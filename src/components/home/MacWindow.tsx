...
const PREVIEW_STORIES = [
  { label: 'SEO · Retail', title: 'Lidl pierde 47 featured snippets de recetas. Mercadona entra por la puerta de atrás' },
  { label: 'Reputación · Turismo', title: 'Vueling acumula 4.200 menciones negativas en X. El 60% viene de un solo hilo de Reddit' },
  { label: 'Señal débil · Moda', title: 'Zara suma 2.3M de búsquedas branded en dos semanas sin campaña visible' },
];
...
export function MacWindow({ headline, deck, sources, timeAgo, formattedDate }: MacWindowProps) {
  ...
  // ── Inicialización: centrar la ventana al montar y al cambiar el viewport ──
  useEffect(() => {
    ...
    const positionWindow = () => {
      ...
      if (!initialized) {
        // Primera vez: centrar con el tamaño por defecto
        setPos({
      ...
      // Resize del viewport: clamp para que no quede fuera
      setPos((p) => ({
  ...
  const onTitleDown = useCallback((e: React.PointerEvent) => {
    if ((e.target as HTMLElement).closest('button')) return; // no arrastrar al pulsar un semáforo
    e.preventDefault();
  ...
  const onTitleMove = useCallback((e: React.PointerEvent) => {
    ...
    const rawX = px + (e.clientX - mx);
    const rawY = py + (e.clientY - my);

    setPos({
      x: Math.min(Math.max(MARGIN, rawX), cw - size.w - MARGIN),
      y: Math.min(Math.max(MARGIN, rawY), ch - size.h - MARGIN),
    });
  }, [dragging, size]);
  ...
  // ── Resize anclado: cada handle modifica solo los lados que toca ──
  ...
  const onResizeMove = useCallback((e: React.PointerEvent) => {
    ...
    if (handle.includes('w')) {
      // mueve el lado izquierdo, ancla el derecho
      l = Math.min(Math.max(MARGIN, left + dx), right - MIN_W);
    }
    if (handle.includes('e')) {
      // mueve el lado derecho, ancla el izquierdo
      r = Math.max(Math.min(cw - MARGIN, right + dx), left + MIN_W);
    }
    if (handle.includes('n')) {
      // mueve el lado superior, ancla el inferior
      t = Math.min(Math.max(MARGIN, top + dy), bottom - MIN_H);
    }
    if (handle.includes('s')) {
      // mueve el lado inferior, ancla el superior
      b = Math.max(Math.min(ch - MARGIN, bottom + dy), top + MIN_H);
    }
    ...
    <div ref={wrapperRef} className="relative w-full h-full">
      {windowState === 'closed' ? (
        ...
        >
          ...
          {/* Content */}
          <div
            className="bg-[#fafaf7] overflow-y-auto"
            style={{ height: windowState === 'open' ? size.h - TITLE_H : 0 }}
          >
            <div className="p-6 sm:p-8">
              <div className="flex items-baseline justify-between border-b border-[#e5e5e0] pb-3 mb-6">
                <span className="font-['Switzer'] font-semibold text-[15px] tracking-[-0.03em] text-[#0a0a0a]">2laps</span>
                <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-[#a3a3a3]">{formattedDate}</span>
              </div>
              <div className="mb-6 pb-6 border-b border-[#e5e5e0]">
                <p className="font-mono text-[9px] uppercase tracking-[0.12em] text-[#a3a3a3] mb-2">BREAKING · RETAIL</p>
                <a href="/2day" className="block group">
                  <h2 className="font-serif font-bold text-[20px] sm:text-[22px] leading-[1.15] text-[#0a0a0a] group-hover:opacity-70 transition-opacity">
                    {headline}
                  </h2>
                </a>
                <p className="mt-2 text-[13px] leading-[1.55] text-[#525252] line-clamp-2">{deck}</p>
                <div className="mt-3 flex items-center gap-3">
                  <SourceChips sources={sources} size="sm" />
                  <span className="text-[11px] text-[#a3a3a3]">·</span>
                  <span className="text-[11px] font-mono text-[#a3a3a3]">{timeAgo}</span>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {PREVIEW_STORIES.map((s) => (
                  <div key={s.title} className="border-t border-[#e5e5e0] pt-3">
                    <p className="font-mono text-[9px] uppercase tracking-[0.1em] text-[#a3a3a3] mb-1.5">{s.label}</p>
                    <p className="font-serif text-[13px] leading-[1.3] text-[#0a0a0a] line-clamp-3">{s.title}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Resize handles — 4 bordes + 4 esquinas, anclados al lado opuesto */}
          {windowState === 'open' && (
            <>
              {/* bordes */}
